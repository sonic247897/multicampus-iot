...
public class User extends Thread{
	...
	// 프로그램 실행할 때 사용하기 위해서 임의로 선언한 객체
	...
	public void ioWork() { //처음 접속했을 때 한 번 실행되는 메소드
		try {
			...
			// checklist는 반복 작업을 하지 않기 위해서 만들어놓은 객체
			...
			// 중복체크(사실 해시맵은 덮어쓰므로 안해줘도 됨)
			if(!check(data[1], checklist)) {
				checklist.put(data[1], this); // this: 소켓 객체
			}
			System.out.println("폰: "+userlist.size());
			System.out.println("카: "+userlist.size());

		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	// 중복 확인 - 기존에 저장된 아이디가 중복저장되지 않도록 하기 위해서 체크 
	...
	private void filteringMsg(String msg) {
		...
		if(protocol.equals("job")) {
			...
			// 메시지를 폰이 보내면 car로 전달해야 함 => 서버에서 결정한다!
